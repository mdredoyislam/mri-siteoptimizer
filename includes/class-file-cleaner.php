...
class File_Cleaner {
	...
	/**
	 * Scan the uploads folder for junk files.
	 * Operations are restricted to the uploads directory only for security.
	 *
	 * @return array
	 */
	public function get_junk_files() {
		$upload_dir = wp_upload_dir();
	 ...
	 * For security, only files inside the uploads directory are allowed.
	 *
	 * @param array $paths
	 * @return array
	 */
	public function delete_files( array $paths ) {
		...
		foreach ( $paths as $path ) {
			...
			// Safety: must be inside the uploads directory only.
			// We do not allow deletion anywhere under the WordPress root (ABSPATH)
			// as that could affect core files, themes, and plugins.
