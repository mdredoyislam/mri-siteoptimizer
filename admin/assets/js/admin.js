/* global MRISiteOptimizerData, jQuery */
(function ($) {
	'use strict';

	const { nonce, ajaxurl, strings } = MRISiteOptimizerData;
	...
	function notice(msg, type, $target) {
		...
		if ($target) {
			...
			$('.mri-siteoptimizer-wrap .so-header').after($n);
		}
	...
	function loadDashboard() {
		...
		ajax('mri_siteoptimizer_get_dashboard_stats', {}).done(function (res) {
	...
	$(document).on('click', '#so-btn-scan-images', function () {
		...
		ajax('mri_siteoptimizer_scan_images', {}, $btn).done(function (res) {
	...
	$(document).on('click', '#so-delete-selected-images', function () {
		...
		ajax('mri_siteoptimizer_delete_images', { ids }, $(this)).done(function (res) {
			if (res.success) {
	...
	$(document).on('click', '#so-btn-bulk-compress', function () {
		const $btn = $(this);
		ajax('mri_siteoptimizer_bulk_compress', { batch: 50 }, $btn).done(function (res) {
			if (res.success) {
	...
	$(document).on('click', '#so-btn-scan-orphans', function () {
		...
		ajax('mri_siteoptimizer_scan_orphans', {}, $btn).done(function (res) {
			if (!res.success) return;
	...
	$(document).on('click', '#so-btn-scan-junk', function () {
		...
		ajax('mri_siteoptimizer_scan_junk', {}, $btn).done(function (res) {
			if (!res.success) return;
	...
	$(document).on('click', '.so-delete-files-btn', function () {
		...
		const action = prefix === 'orphan' ? 'mri_siteoptimizer_delete_orphans' : 'mri_siteoptimizer_delete_junk';
	...
	function loadDbStats() {
		...
		ajax('mri_siteoptimizer_db_stats', {}).done(function (res) {
			if (!res.success) return;
			const s = res.data;
	...
	$(document).on('click', '#so-btn-optimize-db', function () {
		...
		ajax('mri_siteoptimizer_db_optimize', {}, $(this)).done(function (res) {
			if (res.success) {
	...
	$(document).on('click', '#so-btn-save-settings', function () {
		...
		ajax('mri_siteoptimizer_save_settings', data, $(this)).done(function (res) {
	...
	function loadLog() {
		...
		ajax('mri_siteoptimizer_get_log', { limit: 50 }).done(function (res) {
			if (!res.success) return;
			const colors = { scan: '#4F46E5', image_delete: '#EF4444', compress_upload: '#10B981', db_optimize: '#F59E0B', file_delete: '#F59E0B', bulk_compress: '#10B981' };
	...
	$(document).on('click', '#so-quick-scan', function () {
		ajax('mri_siteoptimizer_run_full_scan', {}, $(this)).done(function (res) {
			if (res.success) { notice(res.data.message, 'success'); loadDashboard(); }
		});
	});

	$(document).on('click', '#so-quick-compress', function () {
		ajax('mri_siteoptimizer_bulk_compress', { batch: 50 }, $(this)).done(function (res) {
